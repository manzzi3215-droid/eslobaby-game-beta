...
window.ESLO_CONFIG = {
  ...
  texts: {
    ...
    scenes: {
      ...
      compareEmph: '생분해 케어는 선택이 아니라 필수!',          // v0.10.1: 강조 문구(badge)
      compareTail: '',
  ...
  sfx: {
    enabled: true,
    volume: 0.32,           // v0.10.3: 효과음(SFX) 마스터 볼륨 — 권장 0.30~0.35 중간값(BGM과 밸런스)
    files: {},              // 비어있으면 합성음 사용
    // v0.10.2: 배경음악(BGM) — Web Audio 합성 오리지널 루프(외부 음원·파일 없음, 라이선스 이슈 無).
    //   첫 사용자 인터랙션(게임 시작 버튼) 이후에만 재생, loop, pause/재생 컨트롤과 연동.
    bgm: {
      enabled: true,
      volume: 0.13,         // v0.10.3: BGM 볼륨 — 권장 0.12~0.15 중 낮은 쪽(효과음/안내를 확실히 덜 방해)
    },
  },
