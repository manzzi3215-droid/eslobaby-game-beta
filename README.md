...
현재 버전은 **v0.10.7-beta — PAGE 6·11 영상 삼성 사이니지 다단계 폴백(캐시버스트·SW bypass·load→canplay→play·다중 소스·터치 재생·진단 로그)(총 14페이지)**입니다.
(영상 파일명 변경으로 구 캐시 우회, 서비스워커가 영상·Range 요청 미개입, 1차 실패 시 2차 보수 소스(640×480)로 교체, 그래도 실패 시 터치 재생. PAGE 1~14 음성·효과음·자동 전환은 그대로)
