...
현재 버전은 **v0.10.24-beta — BGM Flip Pro 재생 보강(시작 버튼 click 안에서 BGM 재생 1회 시도, 총 14페이지)**입니다.
(삼성 Flip Pro에서 안내 음성·효과음·영상은 정상인데 BGM만 안 들리는 현상 대응. **기존 BGM 구조·안내 음성·영상·PAGE 7→8 자동전환·PAGE 12 퀴즈·디자인·문구·BGM 파일·볼륨 전부 불변**, `game.js startGame()`에 안내 음성과 동일한 실제 click 제스처로 `SFX.startBGM()` 호출 1줄만 추가. `bgmTryPlay` 재사용이라 이미 재생 중이면 재시작·끊김 없음(currentTime 유지), reject돼도 게임 시작에 영향 없고 기존 pointerdown 언락 fallback 유지. Flip Pro 실기기 검증은 별도 예정.)
(직전 v0.10.23-beta: 속도감 추가 개선 — 안내 음성 1.42배속 + PAGE 6·11 영상 배속 + PAGE 7→8 간헐 멈춤 수정)
(**게임 흐름·페이지 순서·문구·디자인·PAGE 12 퀴즈 로직 불변. 영상 재생 안정성 코드는 리팩터링하지 않음.** ① 안내 음성(PAGE 1~14)을 원본 1.0배속 기준 **1.42배속**으로 재인코딩(v0.10.22의 1.4배속 대체) — ffmpeg `atempo`로 톤 보존, 원본에서 1회 재인코딩(이중 열화 없음). ② PAGE 6·11 영상에 `videoRate`(1.35/1.4) 적용 — 재인코딩 대신 **`playbackRate`**(Flip Pro가 무시해도 1.0 정상 재생 → 재생 실패로 이어지지 않는 안전한 방식, 문제 시 videoRate를 1.0으로 되돌리면 원복). ③ PAGE 7→8 간헐 멈춤 수정 — PAGE 5·7·10·13 음성 자동전환에 **음성 길이 인지 fallback 타이머**를 idempotent하게 추가해 `ended` 이벤트 누락 시에도 자동전환 보장(정상 종료 시 중복 실행 없음). `sw.js` 캐시 버전 상향.)
(직전 v0.10.22-beta: 전체 속도감 개선 — 안내 음성 1.4배속 재인코딩 + 여운·전환·엔딩 등장 딜레이 단축)
(직전 v0.10.21-beta: 관리자 대시보드 개편(기본 비번 0000·비번 변경·간소화·한국시간 날짜별 통계·일 마감/CSV 다운로드) · PAGE 12 '선택' 딥블루화·손가락 확대)
(PAGE 5 경고 제목이 카드 상단에 너무 붙고 아래 설명과 간격이 벌어지던 것 정리 — `.warning-scene` 스코프로 제목을 살짝 내리고(margin-top) 설명과 간격을 좁혀 하나의 경고 메시지처럼, 아기 무대는 남은 공간에서 다시 중앙(screen-body `flex-start` + stage `margin:auto`). 큰 화면(데스크톱·Flip Pro)에서 간격이 82px까지 벌어지던 것도 ~20px로 통일. **텍스트·폰트·색·애니메이션·음성·경고음·자동 진행 불변, 위치·여백만.** PAGE 12 선택 CTA `안심 워시를 선택해주세요.` 색상만 딥 블루 → 따뜻한 앰버-골드(#C8820A)로 변경 — 블루 일색 화면에서 즉시 눈에 띄고 정답 카드 파란 글로우와 구분, 형광 아님·정적, 크기/굵기/박스/카드/로직 불변. CSS만, 신규 자산 없음.)
(직전 v0.10.18-beta: PAGE 12 선택 안내 문구 변경·카드 아래 배치·2번째 줄 행동 강조)
(PAGE 12 선택 안내를 `우리 아이에게 꼭 필요한 / 안심 워시를 선택해주세요.`(2줄)로 바꾸고, 두 카드 **아래**로 이동(DOM: 제목→카드→안내). 2줄을 별도 요소로 분리 — 1줄 `.compare-quiz-hint-sub`(보조·중간 블루), 2줄 `.compare-quiz-hint-action`(`<strong>`·더 굵고 큰 선명한 딥 키 블루로 선택 행동 강조, 정적·글로우 아님). 공통 `.hint` 불변. **상단 제목 `민감한 우리 아이 피부`/질문 `생분해 케어는 선택이 아니라 필수!`는 page12.m4a 음성 일치 위해 유지, 퀴즈 정답·오답 로직·효과음·카드·자동 전환 변경 없음.** 텍스트·DOM·CSS만, 신규 자산 없음.)
(직전 v0.10.17-beta: PAGE 12 선택 안내 문구 강조·2줄화·카드 위 배치)
(PAGE 12 선택 안내를 `두 워시 중 하나를 선택해주세요.` → `두 제품 중 안심하고 사용할 수 있는 / 워시를 선택해주세요.`(2줄)로 바꾸고, 카드 **위**로 옮겨 PAGE 12 전용 클래스 `.compare-quiz-hint`로 강조(공통 `.hint` 불변) — 폰트 확대(clamp)·굵기·딥 블루·둥근 반투명 박스, 360px 2줄 유지. **상단 제목 `민감한 우리 아이 피부` / 질문 `생분해 케어는 선택이 아니라 필수!`는 page12.m4a 음성과 일치 위해 유지, 퀴즈 정답·오답 로직·효과음·카드는 변경 없음.** 텍스트·CSS만 수정, 신규 자산 없음.)
(직전 v0.10.16-beta: PAGE 12 화면 문구를 안내 음성과 일치하도록 원복)
(PAGE 12 안내 음성 `page12.m4a`(v0.10.6 제작)와 화면 텍스트가 어긋나, v0.10.15에서 바꾼 문구를 음성과 맞도록 원복 — 제목 `민감한 우리 아이 피부` / 강조 `생분해 케어는 선택이 아니라 필수!`. **선택형 퀴즈 기능·카드·오답/정답 연출·`두 워시 중 하나를 선택해주세요.` 안내는 v0.10.15 그대로 유지.** 음원은 사용자 제공 TTS라 원본 목소리 복제가 불가해 텍스트를 되돌리는 방식 채택.)
(직전 v0.10.15-beta: PAGE 12 선택형 퀴즈(정답 선택 시에만 진행) · 오답/정답 연출 · 문구 수정)
(PAGE 12를 "두 워시 중 하나 선택" 퀴즈로 변경 — 정답 `착한 계면활성제 이슬로 생분해 워시` 선택 시에만 PAGE 13 자동 이동, 오답 `일반 계면활성제 바디워시`는 오답음+흔들림+붉은 경고 후 페이지 유지·재선택. 진입 시 `quizLocked` 로 탭·다음·이전·페이지점 이동 전부 차단(처음으로만 허용), 카드 전체 터치+키보드 선택. 정답=정답음+파란 강조, 연출(animationend/짧은 fallback) 후 1회만 이동(중복·PAGE 14 스킵 방지). 오답/정답음은 Web Audio 합성(신규 파일 없음), BGM Ducking 연동. 제목 `소중한 우리 아이 피부`/질문 `어떤 선택으로 지켜주시겠어요?`/안내 `두 워시 중 하나를 선택해주세요.`로 문구 교체. **PAGE 5 경보음·6·11 영상·7·10·13 음성 자동 전환·BGM 엔진·Flip Pro has-video 는 변경하지 않음.**)
(직전 v0.10.14-beta: 음성 종료 자동 전환(PAGE 5·7·10) · PAGE 5 경보 알람 · 다음 버튼 표시 회귀 수정 · PAGE 7 문구 위치 재조정)
