...
현재 버전은 **v0.2.7 — 팀 내부 공유용 프로토타입**입니다.
...
### 화면 형태 — 카드뉴스형 (v0.2.7~)
- 중앙의 큰 흰색 카드 배경을 없애고, 욕실 배경 위에 콘텐츠가 자연스럽게 배치됩니다.
- 텍스트(제목·문구 등)에는 작은 반투명 박스/그림자를 적용해 배경 위에서도 잘 보입니다.
- STEP 배지(STEP1~3)는 그대로 유지됩니다.

### 현장 조작 (좌측 컨트롤 / v0.2.2~, v0.2.7 다음 버튼 추가)
...
| → 다음 | 자동 진행 대기 없이 **현재 장면을 건너뛰고 즉시 다음 장면으로** (마지막 장면에서는 비활성화) |

### STEP 표시 방식 (v0.2.5~, v0.2.7 하단 인디케이터)
...
- **하단 페이지 인디케이터**(v0.2.7~)는 `MISSION → STEP1 → STEP2 → STEP3 → MISSION 성공!`
  5개 항목의 작은 원형 표시로, 현재 페이지만 진한 파랑 알약(라벨 표시)·나머지는 연한 파랑 점입니다.
...
- 하단 페이지 인디케이터 항목(MISSION/STEP1~3/MISSION 성공!)을 누르면 해당 구간의 첫 장면으로 바로 이동합니다.
