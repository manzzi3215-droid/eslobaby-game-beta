...
현재 버전은 **v0.10.14-beta — 음성 종료 자동 전환(PAGE 5·7·10) · PAGE 5 경보 알람 · 다음 버튼 표시 회귀 수정(PAGE 1·2·12) · PAGE 7 문구 위치 재조정(총 14페이지)**입니다.
(PAGE 5는 경보 알람(Web Audio 합성) → 안내 음성 → (음성 ended) PAGE 6 자동 전환, PAGE 7·10은 안내 음성 ended 후 PAGE 8·11 자동 전환 — 모두 다음 버튼 숨김·탭 차단·안내 문구 제거, 자동 전환은 실제 음성 ended 기준(임의 타이머 아님), 재진입 시 처음부터 재실행, 오디오 실패 시 안전 fallback. PAGE 1·2·12 다음 버튼이 사라지던 v0.10.13 회귀(lockNext undefined→classList.toggle 토글) 수정. PAGE 7 '안심 베이비케어'가 제품 안쪽까지 과다 침범하던 것을 왼쪽·아래로 소폭 이동(이전 버튼과 겹치지 않고 제품 가장자리에 살짝 걸침). 경보 알람 시작~음성 동안 BGM Ducking 유지. **PAGE 6·11 영상 자동재생·6→7·11→12 자동 이동·PAGE 13 음성 종료 자동 이동·PAGE 3·4·8·9 인터랙션 필수화·BGM·Flip Pro has-video CSS는 변경하지 않음.**)
(직전 v0.10.13-beta: 안내 문구 변경 · PAGE 2 터치 영역 확대 · PAGE 6·11·13 다음 버튼 숨김 · PAGE 7 문구 위치 조정)
(다음 버튼이 있는 페이지의 안내 문구를 "화면을 탭하거나 다음 버튼을 눌러주세요"로 변경(PAGE 6·11 영상 문구·PAGE 13 무문구는 유지). PAGE 2는 일반 바디워시 제품 전체를 탭해도 다음으로 이동(터치 판정 확대, UI 불변). PAGE 6·11(영상 종료 자동 이동)·PAGE 13(음성 종료 자동 이동)은 카드 내부 다음 버튼을 숨김(이전 버튼·자동 전환은 유지). PAGE 7 '안심 베이비케어'를 이전 버튼과 겹치지 않게 오른쪽으로, '생분해'를 제품 이미지 쪽으로 위치만 조정(폰트·크기·애니메이션 불변). **PAGE 6·11 영상 자동재생·6→7·11→12 자동 이동·PAGE 13 음성 종료 자동 이동·PAGE 3·4·8·9 인터랙션 필수화·BGM·Ducking·has-video CSS는 변경하지 않음.**)
(직전 v0.10.12-beta: PAGE 3·4·8·9 인터랙션 필수화(다음 버튼·탭 이동 차단) · 첫 화면부터 BGM 지속 재생(게이트 복귀에도 위치 유지))
(PAGE 3·4·8·9는 문지르기/헹구기 인터랙션을 끝까지 완료해야만 다음으로 자동 전환 — 다음 버튼 숨김·화면 탭·페이지점 이동 전면 차단, 이전 버튼은 유지, 재진입 시 인터랙션 초기화. BGM은 첫 화면(카카오 채널 추가)부터 재생 시도(자동재생 차단 시 첫 제스처에서 시작), 게이트↔게임 이동에도 끊김/재시작 없이 위치 유지, loop·Ducking·정지/재생 유지, 싱글턴. **PAGE 6·11 영상 재생·autoplay·must-watch·has-video CSS·음성 1.2배속·카드 내부 이전/다음 구조는 변경하지 않음.**)
(직전 v0.10.11-beta: 음성 1.2배속·PAGE 13 음성 종료 자동전환·이전/다음 카드 내부 이동·모션 문구 가독성·BGM+Ducking)
...
> v0.6.0-beta: 실제 욕실 배경(세로/가로 반응형·변기 제거 v2) 도입, KEY COLOR(#6DA1FF) 기준 디자인 시스템 통일
> (유리카드·버튼·게이지·타이포·애니메이션 리파인). **게임 로직·문구·관리자 기능 불변.**
