...
현재 버전은 **v0.10.8-beta — PAGE 6·11 영상 자동재생 복원(진입 즉시 autoplay, 실패 확정 시에만 터치 안내)(총 14페이지)**입니다.
(영상 페이지 진입 시 클릭 없이 자동재생 — `autoplay` attribute + JS `play()` 병행, muted 유지. 단순 타임아웃으로 터치 안내를 표시하던 워치독 제거 → Primary·Lo 자동재생이 실제로 모두 실패한 경우에만 터치 안내 표시. 캐시버스트·SW MP4 bypass·다중 소스 폴백·진단 로그·영상 종료 후 자동 전환(6→7, 11→12)·PAGE 1~14 음성은 그대로)
(직전 v0.10.7-beta: PAGE 6·11 영상 삼성 사이니지 다단계 폴백(캐시버스트·SW bypass·다중 소스·터치 재생·진단 로그))
