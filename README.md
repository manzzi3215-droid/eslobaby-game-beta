...
현재 버전은 **v0.10.4-beta — PAGE 14 제품 이미지 최적화(7.5MB→516KB), warning-light 404 제거, precache 반영(총 14페이지)**입니다.
(오디오는 외부 음원 없이 Web Audio로 프로젝트 자체 생성한 오리지널 합성음 — 저작권/라이선스 이슈 없음. BGM 0.13 / 효과음 0.32, 게임 시작 버튼 이후 BGM 재생·loop·정지/재생 컨트롤 연동)
