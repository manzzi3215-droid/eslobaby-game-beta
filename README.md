...
현재 버전은 **v0.9.0-beta — 수동 네비게이션 개편(이전 버튼·자동전환 삭제·탭 진행·플레이/정지 재정의)**입니다.
(컨트롤: 처음으로/플레이/이전/정지/다음, 자동 페이지 전환 완전 삭제, 화면 탭·다음/이전 버튼으로만 이동, 플레이/정지=현재 장면 영상·애니메이션 제어)
(직전 v0.8.0-beta: 신규 2페이지(생분해 설명·Before/After 비교), 총 13페이지)
