...
현재 버전은 **v0.9.2-beta — 후속 UI 수정(마지막 페이지 가로화면 겹침·모바일 상단 로고·하단 컨트롤 정렬)**입니다.
(마지막 페이지: 낮은 가로 화면(≤880px 높이)에서 제목·로고·제품·버튼 겹침/잘림 해소(vh 기준 균형 축소, 세로 불변) · 모바일 상단 브랜드 로고 축소·카드 위 중앙 이동 · 하단 컨트롤 5버튼 정중앙 유지 + 설정 톱니를 컨트롤 위로 올려 겹침 제거)
(직전 v0.9.1-beta: PAGE 6→7 전환 수정 + PAGE 7·12·마지막 페이지 디자인 정리 / v0.9.0-beta: 수동 네비게이션 개편 / v0.8.0-beta: 총 13페이지)
