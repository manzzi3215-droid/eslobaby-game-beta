...
현재 버전은 **v0.9.1-beta — PAGE 6→7 전환 오류 수정 + PAGE 7·12·마지막 페이지 디자인 정리**입니다.
(PAGE 6 영상 영역 탭 시 다음으로 이동되도록 수정(영상 재생/정지는 ▶/⏸ 버튼 유지) · PAGE 7 제품/제품명 확대·키워드 물방울 정리 · PAGE 12 'VS' 삭제·엉덩이 제품 배치 · 마지막 페이지 로고 축소·하단 문구 삭제)
(직전 v0.9.0-beta: 수동 네비게이션 개편 — 처음으로/플레이/이전/정지/다음, 자동 전환 삭제, 탭·버튼 이동 / v0.8.0-beta: 총 13페이지)
