...
현재 버전은 **v0.9.4-beta — 완료 이벤트 기반 자동 전환(6개 장면) + Page6/12 문구·강조**입니다.
(Page3→4·4→5·8→9·9→10 드래그 완료(onComplete) 직후 자동 이동 / Page6→7·11→12 영상 정상 종료(ended) 직후 자동 이동 — 전역/단순 타이머 아님, 실제 완료 이벤트 기반, 수동 이동·중복·이탈 시 예약 취소 · Page6 '나쁜 계면활성제' 붉은 강조 문구 · Page12 좌우 2줄 라벨 균형)
(직전 v0.9.3-beta: Page1 "?"·STEP 정정·필수 시청 영상·Page7 확대·Page10 배지 삭제·모바일 로고 / v0.9.2-beta: 가로 겹침·로고·컨트롤 / v0.8.0-beta: 총 13페이지)
