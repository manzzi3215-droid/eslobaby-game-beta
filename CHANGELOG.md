...
### 1. 안내 음성 1.42배속 (원본 기준 1회 재인코딩)
- PAGE 1~14 음성을 **원본(1.0배속, git `ddb0ce9`) 기준 1.42배속**으로 재인코딩(v0.10.22의 1.4배속 대체). ffmpeg `atempo=1.42`(피치 보존). **1.4배속 파일을 다시 가속하지 않고 원본에서 한 번에** 처리 → 이중 열화 없음.
- 예: page01 4.41→3.11s, page05 6.39→4.49s, page06 6.80→4.79s, page11 5.85→4.12s, page13 5.41→3.83s. 파일명·`config.voice`·재생 로직 불변.

### 2. PAGE 6·11 영상 재생 배속 (빈 시간 단축, 가장 안전한 방식 선택)
- 영상이 음성보다 길어 음성 종료 후 빈 시간 발생(PAGE 6 ~1.7s, PAGE 11 ~3.3s). `scenes.js` 에 **`videoRate`**(PAGE 6=1.35, PAGE 11=1.4) 추가 → `renderVideo` 가 재생 시작 시점(loadedmetadata·play·playing)에만 `video.playbackRate` 적용.
- **방식 선택 근거(Flip Pro 안정성 최우선)**: 영상 **재인코딩은 미채택**(어렵게 안정화한 사이니지 인코딩을 깨뜨릴 위험). **playbackRate 채택** — 인코딩 무변경 + Tizen 이 배속을 무시하면 1.0(정상 속도)으로 재생될 뿐 **재생 실패로 이어지지 않음**(graceful degradation). 기존 autoplay/play()/ended/gate/has-video 경로는 건드리지 않는 **추가 리스너**로만 구현. **문제 재발 시 `videoRate` 를 1.0 으로 되돌리면 즉시 원복.** (실기기 최종 검증 권장.)
- 결과: PAGE 6 지속 ~6.7s→~5.2s, PAGE 11 ~7.7s→~5.9s(브라우저 검증). must-watch·ended 자동전환(6→7, 11→12) 정상 유지.

### 3. PAGE 7→8 간헐 멈춤 수정 (음성 자동전환 공통 구조 보강)
- **원인**: PAGE 5·7·10·13 은 안내 음성 `ended` 이벤트로만 자동전환. autoplay 거부 / Tizen media 이벤트 누락으로 `ended` 가 오지 않으면, 기존 안전망(`armVoiceWatchdog`)은 **잠금만 풀고 자동전환은 하지 않아** 키오스크에서 멈춤. PAGE 13 은 안전망조차 없었음 → **공통 구조 문제**.
- **수정**: 음성 종료 자동전환을 idempotent `autoAdvance` 로 통일하고, **음성 길이 인지 fallback 타이머**(`getVoiceDurationMs` + `음성길이+BUFFER`, 미상 시 상한 9s) 추가 → `ended` 누락 시에도 자동전환 보장. **정상 `ended` 가 항상 먼저 발생**하도록 BUFFER 를 두고, `ended`→`goNext`→`clearScene` 가 fallback 타이머를 취소 → **중복 실행·페이지 건너뜀 없음**(index 가드로도 이중 보호). PAGE 5·7·10·13 공통 적용.
- 검증: 정상 경로(PAGE 7→8 음성 종료 시 이동), `ended` 인위 차단 시에도 fallback 으로 PAGE 7→8 이동, 중복 이동으로 PAGE 8 건너뛰지 않음 확인.

### 4. 전환/여운/애니메이션 타이밍
- v0.10.22 값(페이지 전환 220ms·카드 240ms·완료 여운 300ms·효과음 간격 150ms·엔딩 애니) **검토 후 유지** — 더 줄이면 깜빡임·효과음 잘림 우려. 수학적으로 1.42배 축소하지 않음.

### 5. 캐시
- `sw.js` `CACHE_NAME` = `eslo-game-v0.10.23-beta`(변경된 음성 새로 캐싱).

---

## [v0.10.22-beta] - 2026-07-23
### 전체 속도감 개선 — 안내 음성 1.4배속 재인코딩 + 여운/전환/엔딩 등장 딜레이 단축
**현장 진행 중 "게임이 조금 느리다"는 피드백 반영. 게임 흐름·페이지 구성·문구·정답/오답 로직·자동 전환 규칙·영상 재생(has-video)·드래그 판정은 전부 불변. "속도"만 조정.**

### 1. 안내 음성 1.4배속 재인코딩 (체감 속도의 핵심)
- PAGE 1~14 안내 음성(`assets/audio/voice/page01~14.m4a`)을 **원본(1.0배속) 기준 1.4배속**으로 재인코딩. 기존은 1.2배속이었음 → 실질 약 17% 더 빠름.
- **원본 소스**: git 커밋 `ddb0ce9`(v0.10.6, 무손실 추출 1.0배속)에서 추출 → 이중 열화 없이 **한 번의 재인코딩**으로 1.4배속 생성(음질 유리).
- **피치(목소리 톤) 보존**: ffmpeg `atempo=1.4` 필터 사용(재생 배속 코드 조작 아님) → 삼성 사이니지(Tizen) 등 실기기에서 목소리 톤 왜곡·쪼그라듦 없음. v0.10.11에서 1.2배속 만든 방식과 동일해 안전.
- 파일명·`config.voice` 매핑·재생 로직(`sfx.js`)·음성 종료 자동 전환(PAGE 5·7·10·13) 전부 불변. 예: page01 4.41초→3.15초, page05 6.39→4.56초, page13 5.41→3.88초.

### 2. 여운·전환·엔딩 등장 딜레이 단축 (game.js)
- `AUTO_NEXT_DELAY` **450→300ms**: 드래그(PAGE 3·4·8·9)·영상(PAGE 6·11) 완료 후 다음으로 넘어가기까지 여운 단축.
- `VOICE_SFX_GAP` **250→150ms**: 음성 종료 후 울음/웃음 효과음까지 간격 단축.
- PAGE 13 순차 등장 간격 `STEP` **300→180ms**: 엔딩 제품 소개가 더 빠르게 나타남(자동 전환은 여전히 음성 종료 기준이라 규칙 불변).

### 3. 페이지 전환·카드 등장 애니메이션 단축 (css/game.css)
- `.screen` 페이지 전환 **300→220ms**, 카드 등장 `cardIn` **320→240ms**: 모든 페이지 전환이 조금 더 경쾌.
- PAGE 13/14 엔딩 등장 애니메이션 단축: `revealPop` 520→360ms, `rewardTitlePop` 540→380ms, `rewardFadeScale` 520/240→360/160ms, `rewardFadeUp` 480/560→340/360ms.

### 4. 캐시
- `sw.js` `CACHE_NAME` = `eslo-game-v0.10.22-beta`(변경된 음성 파일이 확실히 새로 캐싱되도록).
